...
export default function App(){
  ...
  const renderScreen=()=>{
    switch(activeTab){
      ...
      case 'PREGAME':  return <MatchPreGame userTeamName={teamName} opponentName={pendingMatch?.opponent||'AI'} difficulty={pendingMatch?.diff||'MEDIUM'} onStart={(diff)=>{setLeagueDifficulty(diff);setActiveTab('MATCH');}} onBack={()=>setActiveTab('HOME')}/>;
      case 'MATCH':    return <MatchScreen squad={squad} onFinish={handleMatchFinish} opponentName={leagueOpponent||undefined} userTeamName={teamName} forcedDifficulty={leagueDifficulty}/>;
  ...
  // Full-screen tabs (no header wrapper)
  if(isMatchTab||activeTab==='PREGAME'){
    return <div style={{height:'100dvh',overflow:'hidden'}}>{screen}</div>;
  }
  return(
